...
like this if you have any uncommitted changes, since this process will change the contents of your project's working directory,
which may erase all of your uncommitted work.
...
![Git Merge Conflict](../FILES/versioncontrol/gitMergeConflict.svg)

Yikes! All of the Python code from both Alice's version and Bob's version are both definitely in there,
...
Here's the full version of my *first.py* file, although yours might not have all of the same conflict markers that mine does:
...
Let's finish this merge off by running `git add first.py`, followed immediately by `git commit`:

![Git Merge Conflict Resolve](../FILES/versioncontrol/gitMergeResolve.svg)

Note that I didn't need to type that commit message.
Git actually provides you with a message if it detects that you are making a merge commit.
...
Now, let's close out our PR from before by pushing our new merge commit:
