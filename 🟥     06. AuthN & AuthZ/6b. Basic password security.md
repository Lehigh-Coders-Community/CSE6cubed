...
Password complexity matters. If you allow your application's users to set really simple passwords like "password123",
then many of them will set horrible passwords like that because they might not be educated in cyber security best practices.
As a result, it's up to you, the application developer, to force these practices upon them.
Both for their own safety and the safety of others.

You probably know the drill with this one, but here's a quick refresher on all of the really annoying,
but incredibly important standards you've probably encountered while making accounts with services on the internet:

- Passwords must be at least 8-9 characters long (but 10-12 characters should be encouraged)
- Passwords should contain a mix of upper- and lower-case letters (at least one of each)
- Passwords should contain numbers
- Passwords should contain at least one special character, like "!", "~", "#", "$", or "%".

These standards enforce passwords that have both high entropy and high complexity,
which can make them almost impossible for an attacker to guess within any reasonable amount of time.

<img src="https://imgs.xkcd.com/comics/password_strength.png" alt="Password Strength, by XKCD" width="500">

(This is a bit of an old comic, which is why "correct horse battery staple" doesn't quite meet modern standards for complexity,
but the point about the importance of high entropy still stands)

### Password Reuse Prevention

Data-breaches rarely announce themselves to you.
Most of the time, you won't even know your database has been jacked until months or even years after the fact.
As a result,  it's best practice to constantly operate under the assumption that not only are there people out there
actively trying to break into your system,
but there's a good chance that somebody has already made off with all of your data without your knowledge.

The best way to cope with this possibility is to regularly prompt your users to change their passwords.
During this process, it's very important to make certain that they don't just reuse their current password,
and you should preferably prevent them from reusing passwords at all.

Many sites will store lists of previously used passwords (in hashed-form!) for each user,
for the express purpose of ensuring that they don't re-use any of their old passwords either.
There will always be ways for them to circumvent these checks (like changing their password 20-30 times
in order to flush your application's password history, before changing right back to their one and only password),
but the point is that most of your users either won't figure this out, or will be to lazy to actively sabotage their own security online.

<img src="https://imgs.xkcd.com/comics/password_reuse.png" alt="Password Reuse, by XKCD" width="240">

Thanks for reading until the end.
In the next chapter, you'll learn OAuth, which is the superior way to handle authentication.
Better read that now, before you forget!

## Where to Next?

- [Next: OAuth](./6c.%20OAuth.md)
